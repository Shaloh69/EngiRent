...
- **Deployed and re-verified — partially.** The pass was subsequently deployed to `desktop-gklhcri` (source files copied directly; the remote's git working tree was left alone, since it carries uncommitted changes from the previous deploy) and all three web surfaces rebuilt there. `docs/design-screenshots/animated-pass/deployed/` holds real screenshots of the **public Cloudflare tunnel URLs**, satisfying §0 for: the Admin Console login in both schemes, and the `client/web` home page. **The Admin dashboard was not re-verified on the deployed instance** — see the Docker gap immediately below. Its light/dark screenshots in `animated-pass/` are from a local production build.
- **The deployed database is down, so anything behind login is unverifiable right now.** The MySQL container has not come back since the machine rebooted: Docker Desktop's Linux engine only starts under an interactive desktop session, and an SSH session can start the `com.docker.service` Windows service but not the engine itself. Until someone opens Docker Desktop on the machine, `POST /auth/login` returns a 500 (`Can't reach database server at 127.0.0.1:3307`). This is an environment/access limitation, not a defect in this pass — the same limitation is already documented for `wsl --install` and the initial `docker pull`.
