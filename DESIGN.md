...
**Phone App, precisely:** the design-system layer is complete — light/dark ThemeData pair, persisted ThemeController defaulting to system, 8px/6px token scales, the animated mesh background, the full type stack, and a set of shared primitives (AppPalette, AppCard, StatusPill, MonoText, SectionLabel, InfoRow, AppEmptyState, Stagger) that screens compose from so spacing and radius cannot drift per screen. **Login and Register are fully rebuilt** against it. **Every screen is now genuinely theme-aware**: 124 direct references to the light-only palette constants across 12 files were resolving through AppColors regardless of brightness, painting light surfaces and near-invisible grey text in dark mode — the same class of bug as the Admin Console original failure. All now resolve through AppPalette.of(context). The Appearance toggle is in the Profile tab per §2.1. **Still outstanding**: the remaining ~10 screens are theme-correct and use the shared primitives where they were touched, but have not each had their layout and component vocabulary reworked against §2.1 the way Login and Register did.
