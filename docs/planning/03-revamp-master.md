...
5. Resolve the two real architectural anomalies: a duplicate in-app admin console, and a fourth client surface (`client/web`) the design mandate doesn't account for.
...
Work through the phases below **in order, in one continuous sweep** — don't pause after each phase for review. Only stop mid-sweep for: the three explicit open questions marked below (duplicate admin console, `client/web` scope, kiosk React-vs-vanilla-JS), anything that turns out to require physical hardware access you don't have, or a Phase 4 audit failure that needs fixing before Phase 5's push. Everything else, keep moving — summarize what changed at the end, not after every phase.
...
- Whether `client/web` also moves to the PC, or stays wherever it currently is (or gets retired, per the still-open Phase 2 question about its scope). Not resolved by the MySQL/Supabase decision above.
...
- **Resolve the duplicate admin console — ask the user first, don't decide silently.** A second, fully independent ~1300-line admin console lives inside the Flutter app (`admin_home_screen.dart`), duplicating `client/admin`'s functionality against the same API with untyped `Map<String,dynamic>` data instead of typed models. Two admin UIs maintained independently against the same backend is a real ongoing cost. Present the option clearly: (a) retire the in-app Flutter admin module entirely, since `client/admin` is the fuller-featured, typed, purpose-built surface, or (b) keep both for a specific reason (e.g. mobile on-the-go admin access) and bring the Flutter one up to parity/consistency instead of leaving it as an independent reimplementation. Do not silently pick one.
- **`client/web` — confirm scope with the user.** It's a real, EngiRent-branded fourth surface (accurate homepage copy, a real `/docs` with four genuinely accurate sections, EngiRent-specific pricing tiers, thesis-relevant blog posts) — not boilerplate, but also not accounted for by the three-surface design mandate. Ask whether it should get its own design pass (and if so, fold it into Phase 3 explicitly) or stay as-is. Two small unmodified scaffold leftovers (`components/counter.tsx`, `package.json`'s `"name": "next-app-template"`) should be cleaned up regardless of that decision.
...
Reference `EngiRent_02_DESIGN_MANDATE.md` in full — it's a mandatory full scrap-and-remake, not incremental restyling, with an enforced screenshot-verify loop and a `DESIGN.md` deliverable. Two things in that document need correcting now that real structure is confirmed, and one open question needs the user's answer before this phase starts:

- **The Kiosk UI is Flask + vanilla JS, not React.** `EngiRent_02_DESIGN_MANDATE.md` §1's animation stack (Framer Motion, React Three Fiber, react-bits) assumes a React-based surface for all three clients. The kiosk genuinely isn't one — it's `server/kiosk/kiosk_ui/`, a local Flask + Socket.io + vanilla-JS touchscreen UI. Two real options, ask the user which:
  - **(a) Migrate the kiosk UI to a small React/Vite static build**, served locally the same way the current Flask static assets are, so it can use the same animation stack (Framer Motion, react-bits, Lottie) as the other two surfaces for full consistency. More upfront work, but one shared design system across all three surfaces.
  - **(b) Keep it vanilla JS and implement the design mandate's intent with vanilla-compatible tools instead** — CSS animations/transitions for most motion, `lottie-web` (the vanilla JS Lottie player, no React needed) for the lock/key micro-animations, and either hand-rolled canvas/CSS 3D or a lightweight vanilla Three.js scene for the idle-screen 3D lock, skipping Spline/React Three Fiber/react-bits specifically since those are React-oriented. Less migration risk, but the kiosk's implementation will diverge somewhat from the other two surfaces' actual code (even while following the same visual spec).
- **`client/web`'s fate is a Phase 2 decision that determines this phase's scope** — if it's kept as a fourth surface needing its own design pass, add it to this phase's screenshot-verify loop explicitly; the design mandate's three-surface palette/token system still applies to it, it just wasn't originally named.
- **The two independently-versioned HeroUI installs** (`client/admin`'s meta-package + React 19, `client/web`'s à-la-carte packages + React 18) become moot for `client/admin` once it moves to Mantine per the design mandate — but if `client/web` stays in scope, decide whether it also moves to Mantine for consistency, or is treated separately since it's simpler, mostly-static content.

Everything else in the design mandate — the three-surface template references, the "EngiRent Spectrum" palette, the kiosk attract-loop pattern, the mandatory `DESIGN.md` + screenshot-verify loop — applies exactly as written.
...
- Don't silently decide the duplicate-admin-console question, the `client/web` scope question (including whether it hosts on the PC), or the kiosk React-migration-vs-vanilla-JS question — none of these get decided without the user's answer.
