...
- **Random every run.** Layout and quote are both re-rolled on each reload and each screen change — on an attract-loop screen variety beats repeatability, and nothing functional depends on the arrangement. The seed is drawn once per run and held in a `useMemo`, never re-rolled per render, or panels jump mid-slide. **The no-repeat guard for quotes must persist in `localStorage`, not a module variable**: a module variable dies with the JS context, so it only prevents repeats within one page session — verification caught the same quote showing twice running across reloads, which is precisely the case that matters on a kiosk that reloads and reboots.
...
- **The exit is the entrance run backwards.** Same slide distance, same duration, same stagger window, reversed order (last panel in is the first out), each panel retreating along the axis it arrived on, with the mirrored easing — reversing `cubic-bezier(x1,y1,x2,y2)` gives `(1-x2, 1-y2, 1-x1, 1-y1)`. The teardown therefore takes exactly as long as the build. Do not give the exit its own animation; the wall must unbuild itself.
- **Panels are glass slabs, not flat rectangles.** A consistent top-left bevel (inset light and dark edges) plus a raked specular sheen, and a per-panel `rotateX`/`rotateY` on the axis of travel under a shared `perspective`, so they swing into place with depth. The lighting direction must be identical on every panel — that consistency is what sells it. **No `backdrop-filter`**: it costs a full-screen blur pass per panel and this runs on a Raspberry Pi driving a 1080x1920 display. The backdrop underneath is a gradient rather than a flat fill, so translucent panels have something to pick up; flat ground makes glass look like paint.
