...
- **Update (2026-08-07, real deployment machine, real `Start.bat`)**: the section above's own blocker — "the local MySQL 8.0 service already running on this machine, no credentials available" — is genuinely, separately resolved now, on the real target machine (`desktop-gklhcri`, via SSH the user provided this session): a disposable Docker MySQL container (`engirent-mysql`, port 3307 — 3306's pre-existing service deliberately left untouched, credentials still unknown and no longer needed) backs a real `DATABASE_URL` in a real `server/node_server/.env`, and **`Start.bat` itself — the actual production launcher, not the dev one — was run for real** (its own commands, since `start`-spawned windows don't persist over this machine's SSH session — see `memory.md`'s session log for why and how this was worked around) and all 4 services came up in genuine `NODE_ENV=production` mode. This is a materially stronger result than the `Start-Dev.bat` re-verification above: the real production build+start path, not the dev server, confirmed via `netstat`, direct HTTP health checks, the Node API's own "Database connected successfully" + "Env: production" log lines, and Playwright screenshots reached directly over Tailscale showing the Admin Console's real (non-demo-mode) login screen. **A genuine, previously-undiscovered bug in `Start.bat` was found and fixed in the process**: `set NODE_ENV=production && npm run build` left a trailing space in the env var (cmd.exe's `set` doesn't trim before `&&`), which failed `env.ts`'s strict Zod validation and would have crashed this exact production launch for every prior user — fixed by quoting the assignment.
- **Verdict: PARTIAL, upgraded again — core CRUD/auth paths and the real production launch itself are now both live-verified on the real target machine.** Still not fully satisfying this section: the client-controlled-payment-amount and kiosk-locker-authorization fixes specifically remain unit-verified only (no real PayMongo-signed webhook was sent, no real/simulated kiosk locker-release authorization attempt was made — both still require items §2 and §4 respectively).
